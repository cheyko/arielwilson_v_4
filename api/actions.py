...
def ypree():
    if request.method == 'POST': 
        ...
        if type_of == 'media':
            ...
            is_media = False
        newPree = Pree(user_id=user_id,is_media=is_media, pree_type=pree_type)
...
def formatPree(pree, user_id):
    ...
    elif pree.pree_type == 'classified':
    ...
    elif pree.pree_type == 'accomadation':
...
@app.route('/api/myglossa', methods=['POST'])
def myglossa():
    if request.method == 'POST':
        user_id = request.json.get('user_id', None)
        results = Preepedia.query.filter(Preepedia.drafter == user_id)
        pages = []
        upload_folder = app.config['UPLOAD_FOLDER'] + "glossa/"
        for page in results:
