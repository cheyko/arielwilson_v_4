...
const MediaItem = props => {
    ...
    // use user_id to determine if user has viewed this media file (hence will have to create views table)
    ...
    useEffect( () => {
        ...
        if (!gotMedia){
            if (aPree.attachment.mediatypes[0] === "audio"){
                setImgView(`${process.env.PUBLIC_URL}/images/exclusives/exclusive${aPree.attachment.exclusive_id}/cover.jpg`);
            }
        ...
        <>
            <div className="card view-user-card">
                <div className="card-content" style={{padding:"0.5rem"}}>
                    <Link className="preepedia-item" to={`/view-blueberry/${pree_id}`}>
                        <div className="media">
...
export default withContext(MediaItem);