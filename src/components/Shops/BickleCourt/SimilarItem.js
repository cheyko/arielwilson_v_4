...
const SimilarItem = (props) => {
    ...
    const matches  = props.context.items ;/*? props.context.products.filter( aProduct => (aProduct.make === aProduct.make 
    ...
    const carousel = matches && matches.length ?
        matches.slice(0,6).map((match, index) => (
            <div onClick={ e => window.scrollTo(0,0)} key={index}>
                <Item
                    imageUrl={process.env.PUBLIC_URL + "/similar/items/item" + match.item_id + "/0"}
                    item={match}
                    key={index}
                    val={index}
                />
            ...
            <div className="hero-body similar-products reduced-column">
                ...
                {matches && matches.length > 0 &&
                    <div className="content has-text-centered">
                        <br />
                        <button className="button is-link"> View Other Similar Item</button>
                    </div>
                }
...
export default withContext(SimilarItem);