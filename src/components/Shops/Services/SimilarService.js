...
const SimilarService = (props) => {
    ...
    const matches  = props.context.services ;/*? props.context.products.filter( aProduct => (aProduct.make === aProduct.make 
    ...
    const carousel = matches && matches.length ?
        matches.slice(0,6).map((match, index) => (
            <div onClick={ e => window.scrollTo(0,0)} key={index}>
                <ServiceItem
                    imageUrl={process.env.PUBLIC_URL + "/images/services/service" + match.service_id + "/0"}
                    service={match}
                    key={index}
                    val={index}
                />
